...
import static org.jbehavesupport.core.verification.VerifierNames.LE;
import static org.springframework.util.Assert.isInstanceOf;
import static org.springframework.util.Assert.isTrue;

@Component
@RequiredArgsConstructor
public final class LowerThanOrEqualVerifier extends AbstractVerifier {
    ...
    public void verify(Object actual, Object expected) {
        ...
        if (((Comparable) actual).compareTo(expectedComparable) > 0) {
            throwAssertionError("value '%s' is not lower than or equal to '%s'", actual, expected);
        }
    }
