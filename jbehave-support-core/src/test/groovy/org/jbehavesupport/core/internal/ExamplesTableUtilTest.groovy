...
class ExamplesTableUtilTest extends Specification {
    ...
    def "should spot duplicates and missing column"() {
        ...
        when:
        ExamplesTableUtil.assertDuplicatesInColumns(table, "data", "verifier")

        then:
        def error = thrown(Error)
        error.getMessage().contains("Examples table contains duplicate value: [value2] in a column [data] in rows: [1, 2]")
        error.getMessage().contains("Examples table doesn't contain column: verifier")
    }
}
